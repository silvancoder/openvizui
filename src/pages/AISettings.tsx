...
import { Typography, Tabs, Card, List, Button, Input, message, Popconfirm, Tag, Empty, Space, Row, Col, Select, Segmented, Modal, Form, Radio } from 'antd';
...
const MoreSettingsTab = ({ t, TOOLS }: { t: any; TOOLS: any[] }) => {
...
Installed Tools:
${TOOLS.map(tool => {
                // We'd ideally check each tool's real path here, but for now just list names
                return `- ${tool.name}`;
            }).join('\n')}
`;
    ...
    const testLocalAI = async () => {
        ...
        try {
            // Simplified check: try to fetch versions or tags
        ...
        try {
            ...
            for (const tool of TOOLS) {
                try {
                    const content = await invoke<string>('get_config_file', { path: tool.configPath });
                    if (content) {
                        allConfigs[tool.name] = content;
                    }
                } catch (e) {
                    console.warn(`Failed to export ${tool.name}:`, e);
                }
            }
