...
const ModelSwitcher: React.FC<ModelSwitcherProps> = ({ toolId }) => {
    ...
    const fetchModels = async () => {
        ...
        let useCliRef = { use: false, provider: '' };
        ...
        if (toolId === 'qoder') {
            ...
            try {
                ...
                if (authContent) {
                    ...
                    if (config && config.fetchFrom && auth[config.fetchFrom]) {
                        targetProvider = auth[config.fetchFrom];
                        providerName = config.fetchFrom;
                    } else {
                        providerName = Object.keys(auth)[0];
                        targetProvider = auth[providerName];
                    }
                    ...
                    if (targetProvider) {
                        ...
                        if (providerName) {
                            ...
                            if (lower.includes('anthropic')) inferredApiType = 'anthropic';
                            else if (lower.includes('google')) inferredApiType = 'google';
                            else inferredApiType = 'openai';
                        }
                    }

                    // Fallback to CLI if address is missing (mirrors OpenCodeAuthEditor behavior)
                    if (!endpoint && providerName) {
                        useCliRef = { use: true, provider: providerName };
                    }
                }
        ...
        try {
            ...
            if (useCliRef.use) {
                ...
                fetchedList = await invoke<string[]>('fetch_remote_models', {
