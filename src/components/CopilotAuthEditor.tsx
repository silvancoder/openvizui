...
const CopilotAuthEditor: React.FC = () => {
    ...
    const runGithubLogin = () => {
        Modal.info({
            title: t('aiSettings.cliConfig.copilotTitle', 'GitHub Copilot Authentication Steps'),
            content: (
                <div style={{ whiteSpace: 'pre-wrap' }}>
                    <p>{t('aiSettings.cliConfig.copilotPrompt')}</p>
                </div>
            ),
            onOk: () => {
    ...
    return (
        ...
        >
            ...
            <div style={{ marginBottom: 24, padding: '16px', background: '#f5f5f5', borderRadius: '8px' }}>
                <Space direction="vertical" style={{ width: '100%' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div>
                            <span style={{ fontWeight: 'bold', fontSize: '16px' }}>{t('aiSettings.cliConfig.sections.auth', 'Authentication')}</span>
                            <p style={{ margin: '4px 0 0', color: 'rgba(0,0,0,0.45)' }}>
                                {t('aiSettings.cliConfig.descriptions.githubAuth', 'Use GitHub CLI to authenticate your account.')}
                            </p>
                        </div>
                        <Button type="primary" icon={<GithubOutlined />} onClick={runGithubLogin}>
                            {t('aiSettings.cliConfig.loginWithGithub', 'Login with GitHub')}
                        </Button>
                    </div>
                </Space>
            </div>
            ...
            <Form form={form} layout="vertical" onFinish={handleSave}>
                <Divider plain>
                    <Space><SettingOutlined />{t('aiSettings.cliConfig.sections.basic', 'Basic Settings')}</Space>
                </Divider>

                <Form.Item name="editor" label={t('aiSettings.cliConfig.fields.editor', 'Editor')} tooltip={t('aiSettings.cliConfig.descriptions.editor')}>
                    <Input placeholder="code" />
                </Form.Item>

                <Space size="large">
                    <Form.Item name="debug" label={t('aiSettings.cliConfig.fields.debugMode', 'Debug Mode')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                    <Form.Item 
                        name="confirm_execute" 
                        label={t('aiSettings.cliConfig.fields.confirmExecute', 'Confirm Before Execute')} 
                        valuePropName="checked"
                        tooltip={t('aiSettings.cliConfig.descriptions.confirmExecute')}
                    >
                        <Switch />
                    </Form.Item>
                </Space>
                ...
                <Divider plain>
                    <Space><BugOutlined />{t('aiSettings.cliConfig.sections.advanced', 'Advanced / Override')}</Space>
                </Divider>

                <Form.Item 
                    name={['github', 'token']} 
                    label={t('aiSettings.cliConfig.fields.githubToken', 'GitHub Token Override')} 
                    tooltip={t('aiSettings.cliConfig.descriptions.githubToken')}
                >
