...
const OpenCodeAuthEditor: React.FC = () => {
    ...
    const loadConfigs = async () => {
        ...
        try {
            ...
            if (authContent) {
                ...
                // Set default fetch provider if not set
                const firstProvider = Object.keys(parsed)[0];
                if (firstProvider) setFetchProvider(firstProvider);
            } else {
                setAuthConfig({});
            }
    ...
    const authTab = (
        <div style={{ padding: '16px 0' }}>
            ...
            <Form.Item
                name="address"
                label={t('aiSettings.cliConfig.opencode.fields.address', 'Address')}
            >
    ...
    const generalTab = (
        <div style={{ padding: '16px 0' }}>
            <Row gutter={16} align="bottom">
                <Col span={12}>
                    <Form.Item label={t('aiSettings.cliConfig.opencode.fields.fetchFrom', 'Fetch Models From')}>
                        <Select 
                ...
                <Col span={12}>
                    <Form.Item>
                        <Button 
                            ...
                            block
                        >
                            {t('aiSettings.cliConfig.opencode.fields.readModels', 'Read Models')}
                        </Button>
                    </Form.Item>
                </Col>
            </Row>
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="theme" label={t('aiSettings.cliConfig.opencode.fields.theme', 'Theme')}>
                        <Select options={[{ label: 'OpenCode', value: 'opencode' }, { label: 'GitHub', value: 'github' }, { label: 'VS Code', value: 'vscode' }]} />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name="autoupdate" label={t('aiSettings.cliConfig.opencode.fields.autoupdate', 'Auto Update')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
            </Row>
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="model" label={t('aiSettings.cliConfig.opencode.fields.model', 'Primary Model')}>
                        <AutoComplete
                ...
                <Col span={12}>
                    <Form.Item name="small_model" label={t('aiSettings.cliConfig.opencode.fields.smallModel', 'Small Model')}>
                        <AutoComplete
                            options={modelList.map(m => ({ value: m }))}
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name="share" label={t('aiSettings.cliConfig.opencode.fields.share', 'Share Mode')}>
                        <Select options={[{ label: 'Manual', value: 'manual' }, { label: 'Auto', value: 'auto' }, { label: 'Disabled', value: 'disabled' }]} />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name="default_agent" label={t('aiSettings.cliConfig.opencode.fields.defaultAgent', 'Default Agent')}>
    ...
    const serverTab = (
        <div style={{ padding: '16px 0' }}>
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name={['server', 'port']} label={t('aiSettings.cliConfig.opencode.fields.serverPort', 'Port')}>
                        <InputNumber style={{ width: '100%' }} min={1} max={65535} />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name={['server', 'hostname']} label={t('aiSettings.cliConfig.opencode.fields.serverHostname', 'Hostname')}>
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name={['server', 'mdns']} label={t('aiSettings.cliConfig.opencode.fields.serverMdns', 'mDNS Discovery')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name={['server', 'mdnsDomain']} label={t('aiSettings.cliConfig.opencode.fields.serverMdnsDomain', 'mDNS Domain')}>
                        <Input placeholder="opencode.local" />
                    </Form.Item>
                </Col>
            </Row>
            <Form.Item name={['server', 'cors']} label={t('aiSettings.cliConfig.opencode.fields.serverCors', 'CORS Allowed Origins')}>
    ...
    const permissionsTab = (
        <div style={{ padding: '16px 0' }}>
            <Alert message={t('aiSettings.cliConfig.opencode.sections.permissions', 'Permission Controls')} type="warning" showIcon style={{ marginBottom: 16 }} />
            <Row gutter={[16, 16]}>
                {[
                    { key: 'edit', label: t('aiSettings.cliConfig.opencode.fields.permEdit') },
                    { key: 'bash', label: t('aiSettings.cliConfig.opencode.fields.permBash') },
                    { key: 'read', label: t('aiSettings.cliConfig.opencode.fields.permRead') },
                    { key: 'grep', label: t('aiSettings.cliConfig.opencode.fields.permGrep') },
                    { key: 'glob', label: t('aiSettings.cliConfig.opencode.fields.permGlob') },
                    { key: 'list', label: t('aiSettings.cliConfig.opencode.fields.permList') },
                    { key: 'skill', label: t('aiSettings.cliConfig.opencode.fields.permSkill') },
                    { key: 'todowrite', label: t('aiSettings.cliConfig.opencode.fields.permTodoWrite') },
                    { key: 'todoread', label: t('aiSettings.cliConfig.opencode.fields.permTodoRead') },
                    { key: 'webfetch', label: t('aiSettings.cliConfig.opencode.fields.permWebFetch') },
                    { key: 'question', label: t('aiSettings.cliConfig.opencode.fields.permQuestion') },
                ].map(p => (
    ...
    const advancedTab = (
        <div style={{ padding: '16px 0' }}>
            <Divider orientation={"left" as any}><AppstoreOutlined /> {t('aiSettings.cliConfig.opencode.sections.advanced', 'Advanced Options')}</Divider>
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name={['tools', 'write']} label={t('aiSettings.cliConfig.opencode.fields.toolsWrite', 'Allow File Write')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name={['tools', 'bash']} label={t('aiSettings.cliConfig.opencode.fields.toolsBash', 'Allow Command Execution')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
            </Row>
            ...
            <Row gutter={16}>
                <Col span={12}>
                    <Form.Item name={['compaction', 'auto']} label={t('aiSettings.cliConfig.opencode.fields.compactionAuto', 'Auto Compaction')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
                <Col span={12}>
                    <Form.Item name={['compaction', 'prune']} label={t('aiSettings.cliConfig.opencode.fields.compactionPrune', 'Prune Context')} valuePropName="checked">
                        <Switch />
                    </Form.Item>
                </Col>
            </Row>
            <Divider />
            <Form.Item name="plugin" label={t('aiSettings.cliConfig.opencode.fields.plugins', 'Enabled Plugins')}>
                <Select mode="tags" style={{ width: '100%' }} placeholder="e.g. opencode-helicone-session" />
            </Form.Item>
            <Form.Item name="instructions" label={t('aiSettings.cliConfig.opencode.fields.instructions', 'Model Instructions')}>
    ...
    const tabsItems = [
        { key: 'auth', label: <span><KeyOutlined /> {t('aiSettings.cliConfig.opencode.sections.auth', 'Auth')}</span>, children: authTab },
        { key: 'general', label: <span><SettingOutlined /> {t('aiSettings.cliConfig.opencode.sections.general', 'General')}</span>, children: generalTab },
        { key: 'server', label: <span><CloudServerOutlined /> {t('aiSettings.cliConfig.opencode.sections.server', 'Server')}</span>, children: serverTab },
        { key: 'permissions', label: <span><SafetyOutlined /> {t('aiSettings.cliConfig.opencode.sections.permissions', 'Permissions')}</span>, children: permissionsTab },
        { key: 'advanced', label: <span><ExperimentOutlined /> {t('aiSettings.cliConfig.opencode.sections.advanced', 'Advanced')}</span>, children: advancedTab },
    ];
