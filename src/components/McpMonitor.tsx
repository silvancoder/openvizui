...
const McpMonitor: React.FC = () => {
    ...
    const inspectServer = async (server: McpServerConfig) => {
        ...
        try {
            ...
            message.success(`${server.key} is online with ${tools.length} tools`);
        } catch (error) {
            console.error('Inspection failed:', error);
            setStatusMap(prev => ({ ...prev, [id]: 'error' }));
            message.error(`${server.key} failed: ${error}`);
        } finally {
    ...
    const showTools = (server: McpServerConfig) => {
        ...
        Modal.info({
            title: `Tools from ${server.key}`,
            width: 600,
            content: (
                <div style={{ marginTop: 16 }}>
                    {tools.length === 0 ? (
                        <Text type="secondary">No tools found or not inspected yet.</Text>
                    ) : (
                        <List
                            dataSource={tools}
                            renderItem={tool => (
    ...
    const columns = [
        {
            ...
            render: (_: any, record: McpServerConfig) => (
                <Space direction="vertical" size={0}>
                    ...
                    <Text type="secondary" style={{ fontSize: 11 }}>{record.toolName} Config</Text>
                </Space>
            )
        },
        {
