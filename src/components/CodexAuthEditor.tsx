...
const CodexAuthEditor: React.FC = () => {
    ...
    const authTab = (
        <div style={{ padding: '16px 0' }}>
            ...
            />
            <Form.Item name="provider" label={t('aiSettings.cliConfig.fields.providerName', 'Provider')} initialValue="openai">
    ...
    const generalTab = (
        <div style={{ padding: '16px 0' }}>
            <Divider orientation={"left" as any}><SettingOutlined /> {t('aiSettings.cliConfig.codex.sections.general', 'General Settings')}</Divider>
            <Space direction="vertical" style={{ width: '100%' }}>
                <Form.Item name="model" label={t('aiSettings.cliConfig.codex.fields.model', 'Model')}>
                    <Input placeholder="deepseek-reasoner" />
                </Form.Item>
                <Form.Item name="model_reasoning_effort" label={t('aiSettings.cliConfig.codex.fields.reasoningEffort', 'Reasoning Effort')}>
                    <Select options={[{ label: 'Low', value: 'low' }, { label: 'Medium', value: 'medium' }, { label: 'High', value: 'high' }]} />
                </Form.Item>
                <Form.Item name="preferred_auth_method" label={t('aiSettings.cliConfig.codex.fields.preferredAuth', 'Preferred Auth')}>
                    <Select options={[{ label: 'API Key', value: 'apikey' }, { label: 'OAuth', value: 'oauth' }]} />
                </Form.Item>

                <Divider orientation={"left" as any}><RocketOutlined /> {t('aiSettings.cliConfig.codex.sections.provider', 'Provider Settings')}</Divider>
                <Form.Item name="model_provider" label={t('aiSettings.cliConfig.codex.fields.modelProvider', 'Provider Type')}>
                    <Select options={[{ label: 'OpenAI', value: 'openai' }, { label: 'Anthropic', value: 'anthropic' }]} />
                </Form.Item>
                <Form.Item name="base_url" label={t('aiSettings.cliConfig.codex.fields.baseUrl', 'Base URL')}>
                    <Input placeholder="https://api.openai.com/v1" />
                </Form.Item>
                <Form.Item name="provider_name" label={t('aiSettings.cliConfig.codex.fields.providerName', 'Provider Name')}>
                    <Input placeholder="openai" />
                </Form.Item>
                <Form.Item name="wire_api" label={t('aiSettings.cliConfig.codex.fields.wireApi', 'Wire API')}>
                    <Input placeholder="responses" />
                </Form.Item>

                <Divider orientation={"left" as any}><RocketOutlined /> {t('aiSettings.cliConfig.opencode.sections.advanced', 'Advanced Settings')}</Divider>
                <Row gutter={16}>
                    <Col span={12}>
                        <Form.Item name="timeout" label={t('aiSettings.cliConfig.codex.fields.timeout', 'Timeout (seconds)')}>
                            <InputNumber style={{ width: '100%' }} />
                        </Form.Item>
                    </Col>
                    <Col span={12}>
                        <Form.Item name="retry_attempts" label={t('aiSettings.cliConfig.codex.fields.retryAttempts', 'Retry Attempts')}>
                            <InputNumber style={{ width: '100%' }} />
                        </Form.Item>
                    </Col>
                </Row>
    ...
    const tabsItems = [
        { key: 'auth', label: <span><KeyOutlined /> {t('aiSettings.cliConfig.codex.sections.auth', 'Auth')}</span>, children: authTab },
        { key: 'general', label: <span><SettingOutlined /> {t('aiSettings.cliConfig.codex.sections.general', 'General')}</span>, children: generalTab },
    ];
